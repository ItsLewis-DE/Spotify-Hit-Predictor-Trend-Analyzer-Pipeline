...
def spotify_pipeline():
   ...
   unload_task = BashOperator(
      task_id = 'unload_task',
      bash_command = 'python3 -u /opt/airflow/dags/scripts/load/unload_file_to_s3.py --date {{ logical_date.strftime("%Y-%m-%d") }}'
   )
   ...
   task_audio_feature >> file_track >> task_artist >> load_top_track_to_s3_task >> load_all_data_s3_to_snow_task >> dbt_run >> unload_task
